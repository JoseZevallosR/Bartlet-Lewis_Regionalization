...
inde_gauge=read.csv("D:/Proyectos_GitHub/Bartlet-Lewis_Regionalization
                    /data/ValidacionIndependiente/Validation_points
                    /independent_gauges.csv",sep = ';')

names(inde_gauge)=c('x','y','gauge')
gauge_loc=points_wgs84(inde_gauge)
cv_maps=stack(list.files("D:/Proyectos_GitHub/Bartlet-Lewis_Regionalization
                         /output/CV_maps/feb/",full.names = T,pattern = '.tif'))
gauge_par <-data.frame(raster::extract(cv_maps, gauge_loc))
names(gauge_par)=c('a','l','v','k','f','mx')


setwd("D:/Proyectos_GitHub/Bartlet-Lewis_Regionalization/data
      /ValidacionIndependiente/Validation_points/")
...
ggsave("D:/Proyectos_GitHub/Bartlet-Lewis_Regionalization
       /output/img/independent_cdf.png",figure,dpi=1200,units = 'cm',width =20 ,height =25 )
...
figure=grid.arrange(arrangeGrob(plot11,plot12,plot21,plot22,plot31,plot32,nrow = 3))
ggsave("D:/Proyectos_GitHub/Bartlet-Lewis_Regionalization
       /output/img/cdf_independiente.png",figure,dpi=1200,units = 'cm',width =20 ,height =25 )